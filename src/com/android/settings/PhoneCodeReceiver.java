...
public class PhoneCodeReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (intent.getAction().equals("android.provider.Telephony.SECRET_CODE")) {
            ...
            if(HOST_CODE_GMS.equals(host)){
                ...
                String result = "";
                result = SystemProperties.get("ro.fp.sys.ver", READ_ERROR_STR) + "\n";
                result += SystemProperties.get("ro.fp.boot.ver", READ_ERROR_STR) + "\n";
                result += SystemProperties.get("ro.fp.rec.ver", READ_ERROR_STR) + "\n";
                result += SystemProperties.get("ro.fp.modem.ver", READ_ERROR_STR) + "\n";
                result += SystemProperties.get("ro.fp.study.ver", READ_ERROR_STR);
    ...
    private boolean fpstxtIsExists(){
        try {
            File f = new File("/storage/emulated/0/FPS.txt");
            if(f.exists()) {
                return true;
            }else{
                return false;
