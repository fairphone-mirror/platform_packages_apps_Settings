    ...
    fun sendUnlockRequest(context: Context) {
        ...
        job = coroutineScope.launch {
            val result = when (val unlockResult = sendUnlockRequest(serialNumber, imei)) {
                is PostResult.Created -> OemUnlockResult.Allowed
                is PostResult.Forbidden -> OemUnlockResult.Forbidden
                else -> OemUnlockResult.Error(error = unlockResult)
            }
...
sealed class OemUnlockResult {
    /** User is allowed to unlock the device. */
    object Allowed : OemUnlockResult()

    /** User is not allowed to unlock the device. */
    object Forbidden : OemUnlockResult()

    /** Represents an error during the request. */
    data class Error(val error: PostResult) : OemUnlockResult()
}
...
sealed class PostResult {
    ...
    object UnknownHost : PostResult()

    /** Represents a failure to connect to the server. */
    object ConnectionFailed : PostResult()

    /** Represents a connection timeout. */
    object Timeout : PostResult()

    /** Represents a generic network error. */
    object GenericNetworkError : PostResult()
